...
camera_offset = [0, -2, 1]  # Offset of the camera relative to the drone
camera_target_offset = [0, 0, 0]  # Offset of the camera target relative to the drone
...
step = 0.1
yaw_step = 0.1  # Step size for yaw rotation
...
keys = {
    ...
    ord('w'): [0, 0, step],           # 'w' key to move up
    ord('s'): [0, 0, -step],          # 's' key to move down
    ord('a'): -yaw_step,              # 'a' key to rotate left
    ord('d'): yaw_step                # 'd' key to rotate right
}
...
while True:
    ...
    # Step the simulation with a fixed time step
    p.stepSimulation()
